  ...
  const response = await fetch("https://api.openai.com/v1/realtime/client_secrets", {
    ...
    body: JSON.stringify({
      session: {
        type: "realtime",
        model: "gpt-realtime",
        instructions: "You are the concise voice interface for the private AiM roadmap. Never claim a consequential action completed without explicit owner confirmation.",
        audio: {
          input: { transcription: { model: "gpt-4o-mini-transcribe", language: "en" }, turn_detection: null },
          output: { voice: "marin" },
        },
