...
<<<<<<< HEAD
  const startVoiceCommand = async () => {
    ...
    try {
      ...
      setOwnerMessage(realtimeSecret ? "Secure Realtime voice session ready. Listening while you hold the button." : "Local speech capture active. Realtime sessions are issued by the deployed API.");
    } catch (error) {
      setOwnerMessage(error instanceof Error ? error.message : "Realtime voice session could not be created.");
      return;
    }
    ...
    if (!recognitionConstructor) {
      setOwnerMessage("Voice input is not supported in this browser. You can still type a command.");
      return;
    }
    ...
    recognition.onresult = (event) => {
      ...
      setManagerRequest(transcript.trim());
    };
    recognition.onerror = () => {
      setVoiceListening(false);
      setOwnerMessage("Voice input could not start. Check microphone permission or type your command.");
    };
  ...
  const toggleManagerSection = (section: string) => {
    setManagerSectionsOpen((current) => current.includes(section) ? current.filter((item) => item !== section) : [...current, section]);
  };
        ...
        <section className="card command-bar" aria-label="Owner command bar">
          <form className="command-form" onSubmit={runManagerDraft}>
            <label className="command-field">
              ...
              <textarea className="command-input" maxLength={4000} onChange={(event) => setManagerRequest(event.target.value)} placeholder="Write a command for your Manager…" required rows={1} value={managerRequest} />
            </label>
            <button aria-label="Send command" className="command-send" disabled={ownerBusy || !managerRequest.trim()} type="submit"><Send className="icon-small" /><span>{ownerBusy ? "Working…" : "Send"}</span></button>
            <button aria-label="Hold to talk" className={`command-talk ${voiceListening ? "command-talk-active" : ""}`} onPointerDown={startVoiceCommand} onPointerLeave={stopVoiceCommand} onPointerUp={stopVoiceCommand} type="button"><Mic className="icon-small" /><span>{voiceListening ? "Listening…" : "Hold to talk"}</span></button>
          </form>
